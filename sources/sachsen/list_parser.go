...
const defaultListSelector = `a[href*="revosax"], a[href*="vorschrift"], a[href*="dokument"], a[href*="id="]`
...
func parseListDocumentRefs(baseURL string, raw []byte, selector string) ([]godr.DocumentRef, error) {
	...
	doc.Find(selector).Each(func(i int, sel *goquery.Selection) {
		...
		if resolved == "" || !isLikelySachsenDocumentURL(resolved) {
			return
		}
		...
		id := deriveSachsenID(resolved)
		if id == "" {
			id = fmt.Sprintf("sachsen-%d", len(refs)+1)
		}
...
func deriveSachsenID(v string) string {
	u, err := url.Parse(strings.TrimSpace(v))
	if err != nil {
		return ""
	}
	...
	q := u.Query()
	for _, key := range []string{"id", "doknr", "docid", "j"} {
