...
const (
	// DefaultListURL is a practical entry page for VORIS listing/search.
	DefaultListURL = "https://voris.wolterskluwer-online.de/"
)

// Source implements Niedersachsen ingestion from VORIS.
type Source struct {
	Client       HTTPClient
	Parser       Parser
...
// FetchDocument downloads one Niedersachsen document page and parses it into a LegalDocument.
...
func isLikelyNiedersachsenDocumentURL(v string) bool {
	u, err := url.Parse(v)
	if err != nil {
		return false
	}
	host := strings.ToLower(u.Host)
	if !strings.Contains(host, "voris") {
		return false
	}
	pathLower := strings.ToLower(u.Path)
	queryLower := strings.ToLower(u.RawQuery)
	return strings.Contains(pathLower, "document") || strings.Contains(pathLower, "voris") || strings.Contains(queryLower, "docid=") || strings.Contains(queryLower, "id=") || strings.Contains(queryLower, "d=")
}
