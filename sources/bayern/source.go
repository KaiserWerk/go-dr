...
const (
	// DefaultListURL is a practical entry page for BAYERN.RECHT listing/search.
	DefaultListURL = "https://www.gesetze-bayern.de/"
)

// Source implements Bayern ingestion from gesetze-bayern.de.
type Source struct {
	Client       HTTPClient
	Parser       Parser
...
// FetchDocument downloads one Bayern document page and parses it into a LegalDocument.
...
func isLikelyBayernDocumentURL(v string) bool {
	u, err := url.Parse(v)
	if err != nil {
		return false
	}
	host := strings.ToLower(u.Host)
	if !strings.Contains(host, "gesetze-bayern.de") {
		return false
	}
	pathLower := strings.ToLower(u.Path)
	queryLower := strings.ToLower(u.RawQuery)
	return strings.Contains(pathLower, "/content/document") || strings.Contains(pathLower, "/document") || strings.Contains(queryLower, "id=") || strings.Contains(queryLower, "docid=")
}
