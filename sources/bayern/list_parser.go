...
const defaultListSelector = `a[href*="gesetze-bayern"], a[href*="Content/Document"], a[href*="document"], a[href*="id="]`
...
func parseListDocumentRefs(baseURL string, raw []byte, selector string) ([]godr.DocumentRef, error) {
	...
	doc.Find(selector).Each(func(i int, sel *goquery.Selection) {
		...
		if resolved == "" || !isLikelyBayernDocumentURL(resolved) {
			return
		}
		...
		id := deriveBayernID(resolved)
		if id == "" {
			id = fmt.Sprintf("bayern-%d", len(refs)+1)
		}
...
func deriveBayernID(v string) string {
	u, err := url.Parse(strings.TrimSpace(v))
	if err != nil {
		return ""
	}
	...
	q := u.Query()
	for _, key := range []string{"id", "docid", "doknr", "j"} {
