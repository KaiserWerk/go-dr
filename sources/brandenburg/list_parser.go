...
const defaultListSelector = `a[href*="bravors"], a[href*="doknr"], a[href*="id="], a[href*="dokument"]`
...
func parseListDocumentRefs(baseURL string, raw []byte, selector string) ([]godr.DocumentRef, error) {
	...
	doc.Find(selector).Each(func(i int, sel *goquery.Selection) {
		...
		if resolved == "" || !isLikelyBrandenburgDocumentURL(resolved) {
			return
		}
		...
		id := deriveBrandenburgID(resolved)
		if id == "" {
			id = fmt.Sprintf("brandenburg-%d", len(refs)+1)
		}
...
func deriveBrandenburgID(v string) string {
	u, err := url.Parse(strings.TrimSpace(v))
	if err != nil {
		return ""
	}
	...
	q := u.Query()
	for _, key := range []string{"doknr", "id", "docid", "j"} {
