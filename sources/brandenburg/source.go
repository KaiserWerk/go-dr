...
const (
	// DefaultListURL is a practical entry page for BRAVORS listing/search.
	DefaultListURL = "https://bravors.brandenburg.de/"
)

// Source implements Brandenburg ingestion from bravors.brandenburg.de.
type Source struct {
	Client       HTTPClient
	Parser       Parser
...
// FetchDocument downloads one Brandenburg document page and parses it into a LegalDocument.
...
func isLikelyBrandenburgDocumentURL(v string) bool {
	u, err := url.Parse(v)
	if err != nil {
		return false
	}
	host := strings.ToLower(u.Host)
	if !strings.Contains(host, "bravors.brandenburg.de") {
		return false
	}
	pathLower := strings.ToLower(u.Path)
	queryLower := strings.ToLower(u.RawQuery)
	return strings.Contains(pathLower, "dokument") || strings.Contains(pathLower, "bravors") || strings.Contains(queryLower, "doknr=") || strings.Contains(queryLower, "id=")
}
